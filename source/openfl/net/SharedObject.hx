...
class SharedObject extends EventDispatcher
{
	...
	**/
	public static function getLocal(name:String, localPath:String = null, excludeCompany:Bool = false, secure:Bool = false /* note: unsupported**/):SharedObject
	{
		var illegalValues = [" ", "~", "%", "&", "\\", ";", ":", "\"", "'", ",", "<", ">", "?", "#"];
		var allowed = true;
		...
		if (!__sharedObjects.exists(id))
		{
			...
			try
			{
				...
				var path = __getPath(localPath, name, excludeCompany);
	...
	**/
	...
	@:noCompletion private static function __getPath(localPath:String, name:String, excludeCompany:Bool = false):String
	{
		#if lime
		...
		if (excludeCompany) path = StringTools.replace(path, 'ShadowMario' + '\\', '');
		//trace(path + name + '.sol');
