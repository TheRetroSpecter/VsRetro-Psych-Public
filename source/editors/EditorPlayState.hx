...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		PlayState.arrowSkinbf = PlayState.SONG.arrowSkinbf;
		PlayState.arrowSkindad = PlayState.SONG.arrowSkindad;
		PlayState.splashSkin = PlayState.SONG.splashSkin;

		if(ClientPrefs.noteQuantization) {
			PlayState.arrowSkinbf = "NOTE_assets_quant";
			PlayState.arrowSkindad = "NOTE_assets_quant";
		}
		...
		if(ClientPrefs.noteSplashes) {
			if(PlayState.splashSkin == null) {
				if(PlayState.arrowSkinbf == "NOTE_assets_retrobf") {
					PlayState.splashSkin = "noteSplashes_retrobf";
				}

				if(PlayState.arrowSkinbf == "NOTE_assets_quant") {
					PlayState.splashSkin = "noteSplashes_quant";
				}
			}
		...
		var tipText:FlxFixedText = new FlxFixedText(10, FlxG.height - 24, 0, 'Press ESC to Go Back to Chart Editor', 16);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var speed = FlxMath.roundDecimal(PlayState.SONG.speed, 2);
			...
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if(daStrumTime >= startPos) {
						...
						swagNote.angle = swagNote.offsetAngle + swagNote.offsetAngleQuant;
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					StrumPlayAnim(true, daNote, time);
	...
	{
		var skin = player == 1 ? PlayState.arrowSkinbf : PlayState.arrowSkindad;
		...
		for (i in 0...4)
		{
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X, strumLine.y, i, player, skin);
