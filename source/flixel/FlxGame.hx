...
class FlxGame extends Sprite
{
	...
	/*{
		if((_requestedState is PlayState)) {
			if (playStateThread == null) {
				playStateThread = Thread.create(function() {
					while (true) {
						//if (!threadActive)
						//{
						...
						var state:Null<Int> = Thread.readMessage(false);
						if (state != null) {
							loadingState = true;
							_switchState();
							var ps:PlayState = cast _requestedState;
							ps.loaded = true;
							loadingState = false;
						}
					}
