...
function onStepHit()
	if not(doingEnough) and curStep >= 247 and curStep < 256 then --  and get('camZooming')
	...
	if curStep >= 249 and not saidEnough then
	...
	if curStep >= 247 and curStep < 256 then
		set('camZooming', false)
	end

	if not(stoppedEnough) and curStep >= 256 then
